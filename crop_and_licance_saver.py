...
def crop_and_save_plate(img, box, plate_number, conf,output_folder='output'):
    ...
    car_img_path = os.path.join(output_folder, f'car_{plate_number}_{conf}.jpg')
    cv2.imwrite(car_img_path, img)
    print(f"Car image saved at {car_img_path}")

    # Save the cropped plate image
    plate_img_path = os.path.join(output_folder, f'plate_{plate_number}_{conf}.jpg')
